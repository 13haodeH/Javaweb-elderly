 ...
 */
@WebServlet("/manage/admin_doworkeradd")
public class DoWorkerAdd extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//成功或失败重新定向
			...
			if(count > 0) {
				response.sendRedirect("admin_doworkerselect");
			}else {
				PrintWriter out = response.getWriter();
				
				out.write("<script>");
				out.write("alert('用户添加失败');");
				out.write("location.href='admin_workeradd.jsp';");
				out.write("</script>");
			}
	}
