...
/**
 * Servlet implementation class DoHomeDel
 */
@WebServlet("/manage/admin_dohomedel")
public class DoHomeDel extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//设置字符集
				...
				//加入到数据库用户表中
				int count = elderly_homerDao.del(name);
				...
				//成功或失败重新定向
					...
					if(count > 0) {
						...
						out.write("location.href='admin_dohomeselect?cp="+request.getParameter("cpage")+"'");
						out.write("</script>");
					}
	}
