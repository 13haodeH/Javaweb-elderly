...
/**
 * Servlet implementation class DoDrugAdd
 */
@WebServlet("/manage/admin_dodrugadd")
public class DoDrugAdd extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//加入到数据库用户表中
		int count = elderly_drugDao.insert(e);
		
		
		//成功或失败重新定向
			
			if(count > 0) {
				response.sendRedirect("admin_dodrugselect");
			}else {
				PrintWriter out = response.getWriter();
				
				out.write("<script>");
				out.write("alert('用户添加失败');");
				out.write("location.href='admin_drugadd.jsp';");
				out.write("</script>");
			}
	}
